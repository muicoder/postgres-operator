...
RUN apk add --no-cache autoconf automake c-ares-dev curl gcc libc-dev libevent-dev libtool make openssl-dev pkgconf-dev && \
  curl -sL https://www.pgbouncer.org/downloads/files/1.24.1/pgbouncer-1.24.1.tar.gz | tar -xzv && \
  cd pgbouncer-* && \
  curl -sL https://github.com/pgbouncer/pgbouncer/archive/ee391b6042eca627d7986aab5e9e0b05538c3d92.tar.gz | tar -xzv --strip-components=1 && \
